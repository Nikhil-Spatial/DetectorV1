...
- Due to memory and GPU restrictions, I resized every image to 224 x 224 instead of the 448 x 448 described in the YOLOv1 paper. This decision was made to speed up training time.
- I normalized the image pixel values to the range [0, 1] by dividing each pixel value by 255.
- I standardized the image pixels using the mean and standard deviations of the ImageNet-1k dataset (see *Model Architecture*).

## 👷 Model Architecture 
- The backbone is the ResNet-18 model pretrained on the ImageNet-1k dataset. The model's classification head was cut, specifically the global average pooling and fully connected layers.
...
- In the bounding box portion of the prediction vector, the x, y, and confidence score predictions are activated using a sigmoid layer to map, potentially, negative values to the range [0, 1].
...
- I used the AdamW optimizer with a decay of 5e-4. The initial learning rates of the backbone and detector head are 1e-4 and 1e-3, respectively.
